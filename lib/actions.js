...
export async function createBranch(data) {
  ...
  const { name, city, state, address, phone, email, description, images, amenities, pricePerSlot } = data;
  ...
  await db.slot.createMany({
    data: [
      { branchId: branch.id, title: "Morning Slot", startTime: "09:00", endTime: "13:00", price: parseFloat(pricePerSlot) || 5000 },
      { branchId: branch.id, title: "Afternoon Slot", startTime: "14:00", endTime: "18:00", price: parseFloat(pricePerSlot) || 5000 },
      { branchId: branch.id, title: "Evening Slot", startTime: "19:00", endTime: "23:00", price: (parseFloat(pricePerSlot) || 5000) * 1.2 },
    ]
  });
...
export async function assignManagerToBranch(userId, branchId) {
  ...
  // Update user role to MANAGER and assign branch
...
export async function createOnlineBooking(data) {
  ...
  const { branchId, slotId, bookingDate, customerName, customerEmail, customerPhone, notes } = data;
  ...
  const amount = slot ? slot.price : branch.pricePerSlot;
