...
export default async function AdminDashboardPage() {
  ...
  const pendingInquiries = await db.franchiseInquiry.count({
  ...
  const franchiseRevenueList = branches.map((b) => {
    const onlineBookings = b.bookings.filter(
      (bk) => bk.bookingType === "ONLINE",
    );
    const offlineBookings = b.bookings.filter(
      (bk) => bk.bookingType === "OFFLINE",
    );

    const onlineRevenue = onlineBookings.reduce(
      (sum, bk) => sum + bk.totalAmount,
      0,
    );
  ...
  allBookings.forEach((b) => {
    ...
    if (b.bookingType === "ONLINE")
      monthlyRevenueMap[monthKey].online += b.totalAmount;
      ...
      <div className="container mx-auto max-w-6xl">
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-gray-500">
                Revenue breakdown by each franchise location (Online vs Offline
                walk-ins).
              </p>
            </div>
                  ...
                  >
                    ...
                    <td className="py-4 px-3 text-xs">
                      <span className="font-semibold">
                        {f.onlineCount} bookings
                      </span>
                      <div className="text-gray-400">
                        ₹{f.onlineRevenue.toLocaleString()}
                      </div>
                    </td>
                    <td className="py-4 px-3 text-xs">
                      <span className="font-semibold">
                        {f.offlineCount} walk-ins
                      </span>
                      <div className="text-gray-400">
                        ₹{f.offlineRevenue.toLocaleString()}
                      </div>
                    </td>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-3xl p-6 border border-gray-200 dark:border-gray-800 shadow-xs mb-10">
          ...
          <p className="text-xs text-gray-500 mb-4">
            Month-by-month earnings timeline across all franchise branches.
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <Link
            href="/admin/branches"
            className="p-6 rounded-3xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 hover:border-rose-500 transition-all shadow-xs group"
          >
            <div className="text-3xl mb-2">🏢</div>
            <h3 className="text-xl font-bold text-gray-900 dark:text-white group-hover:text-rose-600">
              Franchise Branches
            </h3>
            <p className="text-xs text-gray-500 mt-1">
              Add new branches, set pricing, update photos & city locations.
            </p>
          </Link>
          ...
          >
            ...
            <p className="text-xs text-gray-500 mt-1">
              Promote registered users to Manager and assign them to branches.
            </p>
          </Link>
          ...
          >
            ...
            <p className="text-xs text-gray-500 mt-1">
              Review applications from prospective franchise partners.
            </p>
          </Link>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-3xl p-6 border border-gray-200 dark:border-gray-800 shadow-xs">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-4">
            Latest System Bookings
          </h2>
          {recentBookings.length === 0 ? (
            <p className="text-sm text-gray-500 py-6 text-center">
              No bookings logged in the system yet.
            </p>
          ) : (
            <div className="space-y-3">
              {recentBookings.map((b) => (
                <div
                  key={b.id}
                  className="p-4 rounded-2xl border border-gray-100 dark:border-gray-800 flex items-center justify-between"
                >
                  <div>
